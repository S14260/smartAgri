;!function (win) {
    ...
    Xadmin.prototype.init = function (layFilter) {
        layFilter = layFilter || 'xbs_tab';
        var tab_list = this.get_data();
        for (var i in tab_list) {
            this.add_lay_tab(tab_list[i].title, tab_list[i].url, i, layFilter);
        }
        element.tabChange(layFilter, i);
    };
    ...
    Xadmin.prototype.add_tab = function (title, url, is_refresh, layFilter) {
        layFilter = layFilter || 'xbs_tab'; // 默认老版行为

        var id = md5(url); // md5 每个 url

        // 获取对应 tab 容器下的 iframe
        var $iframes = $('.layui-tab[lay-filter="' + layFilter + '"] .x-iframe');

        // 重复点击
        for (var i = 0; i < $iframes.length; i++) {
            if ($iframes.eq(i).attr('tab-id') == id) {
                element.tabChange(layFilter, id);
                if (is_refresh) $iframes.eq(i).attr("src", $iframes.eq(i).attr('src'));
                return;
            }
        }

        // 新增 tab
        this.add_lay_tab(title, url, id, layFilter);
    ...
    Xadmin.prototype.add_lay_tab = function (title, url, id, layFilter) {
        layFilter = layFilter || 'xbs_tab'; // 默认老版行为
...
layui.use(['layer', 'element', 'jquery'], function () {
    ...
    // === 每次登录后清空 tab 缓存 ===
    layui.data('tab_list', null); // 清空所有 tab 缓存

    // 打开页面初始（此时 tab 栏为空）
    xadmin.init('adminTab');
    xadmin.init('userTab');

    // 关闭 tab 时同步删除缓存
    ['adminTab', 'userTab'].forEach(function (filter) {
        element.on('tabDelete(' + filter + ')', function (data) {
            var layId = data.elem.attr('lay-id'); // 获取被关闭的 tab id
            xadmin.del_data(layId); // 删除缓存
        });
    });
